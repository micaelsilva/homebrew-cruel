class PythonAT310 < Formula
  ...
  def lib_cellar
    prefix/"Frameworks/Python.framework/Versions/#{version_major_minor}/lib/python#{version_major_minor}"
  end
  ...
  def install
    ...
    (prefix/"Frameworks/Python.framework/Versions/#{version_major_minor}/lib/python#{version_major_minor}/site-packages").rmtree
  ...
  def sitecustomize
    ...
    <<~EOS
      ...
      tkinter_prefix = "#{HOMEBREW_PREFIX}/opt/python-tk@#{version_major_minor}/libexec"
  ...
  def caveats
    xy = version.to_s.slice(/(3\.\d+)/) || "3.10"
    <<~EOS
      Python has been installed as
        #{HOMEBREW_PREFIX}/bin/python#{version_major_minor}

      Unversioned and major-versioned symlinks `python`, `python3`, `python-config`, `python3-config`, `pip`, `pip3`, etc. pointing to
      `python#{version_major_minor}`, `python#{version_major_minor}-config`, `pip#{version_major_minor}` etc., respectively, have been installed into
        #{opt_libexec}/bin

      You can install Python packages with
        pip#{version_major_minor} install <package>
